...
def main(argv):
    ...
    # ---------------------------------------------------------------- 7
    it.item(7, "Acesso ao recuo da sala de apoio (parede oeste, de O1 à parede norte) livre de mesa")
    # Desde 22/09 a sala de apoio e um recuo para dentro da parede, fora do piso:
    # nao ha zona protegida, mas nenhum corpo de mesa pode passar da borda norte
    # de O1 (y = 38,50), onde comeca o acesso a ela.
    o1 = portas["O1"]
    limite = o1["y2"]
    if any(z.get("tipo") == "sala_apoio" for z in zonas):
        it.erro("decisoes.zonas_protegidas ainda traz a sala de apoio como zona do piso (superado em 22/09)")
    invade = [n for n, m in pos.items()
              if m["rot"] == 0 and retangulo(m)[3] > limite + TOL]
    if invade:
        it.erro(f"mesas {sorted(invade)} com corpo acima de y={limite} (acesso ao recuo)")
    else:
        topo = max((retangulo(m)[3] for m in pos.values() if m["rot"] == 0), default=0)
        it.ok(f"nenhuma mesa da parede oeste passa de y={limite}; a mais ao norte termina em y={topo:.2f}")
