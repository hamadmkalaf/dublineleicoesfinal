...
# Revisao de 22/09/2026: a "sala de apoio" nao e piso do salao -- e um recuo
# para dentro da parede oeste, entre a porta O1 e a parede norte, e por isso
# deixou de ser zona protegida. O trecho 2 da parede oeste, que terminava em
# 36,25 (borda sul da O1 menos meio modulo), vai ate 39,25: como o bloco de
# unidades e centrado no trecho, isso desloca A3, A4 e A5 1,50 m para o norte
# -- a mesa 3313·3889 sai de 0,24 para 1,74 m do recuo da O2, e a ultima mesa
...
PAREDES = {
    "oeste": {"rot": 0,   "eixo": "y", "fixo": 0.0,  "sentido": +1,
              "trechos": [(7.45, 18.81), (22.98, 39.25)]},
...
def arranja_parede(parede, unidades, verde_de=lambda n: True, zonas=()):
    ...
    por_mrvs = {frozenset(u["mrvs"]): u for u in unidades}
    # A ordem fixada so se aplica quando a parede tem exatamente essas unidades
    # (a busca da reparticao otima testa outras composicoes, que seguem livres).
    if parede in ORDEM_FIXA and set(map(frozenset, sum(ORDEM_FIXA[parede], []))) == set(por_mrvs):
        seqs = []
        for trecho in ORDEM_FIXA[parede]:
            seq = []
            for ns in trecho:
                u = dict(por_mrvs[frozenset(ns)], mrvs=list(ns))   # a ordem dentro da dupla da o lado
                seq.append(u)
            seqs.append(seq)
        for (a, b), seq in zip(trechos, seqs):
            assert largura(seq, vao_dupla) <= (b - a) + 1e-9, (parede, seq, "nao cabe no trecho")
        postas = _coloca(trechos, seqs, vao_dupla)
        nota = _avalia(parede, postas, seqs, vermelha, verde_de, zonas, para_salao)
        assert nota is not None, f"a ordem fixada da parede {parede} viola as regras duras"
    fora = []
...
def monta(mesas, grupos, altas):
    ...
    for parede in ORDEM:
        ...
        zonas = [z["rect"] for z in ZONAS_FIXAS]
...
# Cor de cada classe de carga nas plantas geradas daqui (decisoes.classes.cores e
# mesas[].cor). Em 22/09 a classe alta passou de vermelho #c0392b a roxo: o
# vermelho virou a cor da zona C na sinalizacao e no layout dos separadores.
CORES_CLASSE = {
    "alta":  {"cor": "roxo",    "hex": "#6A1B9A", "rotulo": "alto comparecimento"},
...
def sinalizacao_e_zonas(planta, decisoes):
    ...
    # A sala de apoio (recuo entre O1 e a parede norte) nao e zona do piso;
    # o que se protege e o acesso a ela, e isso e conferido em confere_arranjo.
    return sinal, zonas
...
def grava(decisoes, grupos, arranjo, quando="2026-09-22"):
    ...
    for m in decisoes["mesas"]:
        ...
        m["cor"] = CORES_CLASSE[m["classe"]]["hex"]
    decisoes["classes"]["cores"] = {k: dict(v) for k, v in CORES_CLASSE.items()}
    ...
    decisoes["entradas_por_parede"] = (
        ...
        "por cota do Ring 3: as três zonas do pátio passaram a ser dimensionadas iguais, uma por parede.")
    ...
    if "ring3" in decisoes:
        decisoes["ring3"] = {
            "situacao": "pátio de fila vigente, confirmado em 18/09/2026 — recinto ao ar livre de "
                        "44,0 × 35,0 m ao sul do Hall 2, três zonas iguais de 23 raias, uma por parede "
                        "(A oeste, B norte, C leste). Revisão de 22/09: boca da A a oeste, saída da B "
                        "no meio, quatro aberturas na face norte. Montagem em mapa/ring3_montagem.html.",
            "decidido_em": "2026-09-18",
            "revisado_em": "2026-09-22",
            "montagem": "scripts/ring3_montagem.py (hamadmkalaf/eleicoes2026) → saidas/ring3_montagem.json",
            "larguras_das_zonas": "iguais, 12,87 / 12,86 / 12,87 m; as cotas do Ring 3 antigo (3642/4215/3642) não valem mais",
        }
